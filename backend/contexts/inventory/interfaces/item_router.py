...
@router.post(
    ...
    summary="Create item asynchronously",
)
async def create_item_async(
    publisher: InventoryCommandPublisher,
    command: CreateItemCommand = Body(...),  # noqa: B008
):
    try:
        created_item = await publisher.publish_create_item(command)
        return ItemDTO.model_validate(created_item)
    except DomainError as e:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(e))  # noqa: B904
    except Exception as e:
        print(f"Error creating item asynchronously: {e}")
        raise HTTPException(  # noqa: B904
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An error occurred during item creation.",
        )
...
@router.put(
    ...
    response_model=ItemDTO,
    status_code=status.HTTP_202_ACCEPTED,
    summary="Update item asynchronously",
)
async def update_item_async(
    ...
    try:
        command.id = item_id  # Set the ID for the update
        updated_item = await publisher.publish_update_item(command)
        return ItemDTO.model_validate(updated_item)
    except EntityNotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))  # noqa: B904
    except Exception as e:
...
@router.delete(
    "/async/{item_id}",
    status_code=status.HTTP_202_ACCEPTED,
    summary="Delete item asynchronously",
)
async def delete_item_async(
    ...
    try:
        command = CreateItemCommand(item_id=item_id)
        await publisher.publish_delete_item(command)
    except EntityNotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))  # noqa: B904
    except Exception as e:
        print(f"Error deleting item {item_id}: {e}")
